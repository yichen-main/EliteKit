...
internal static class AttackExpand
{
    ...
    public static async ValueTask CreateAsync<T1, T2>(Action<Exception> e) where T1 : PlatformBuilder where T2 : FactoryBuilder
    {
        try
        {
            ...
            {
                return x.GetInterfaces().Contains(typeof(IEditionConstructor)) && x.IsClass;
            })) services.AddKeyedSingleton(typeof(IEditionConstructor), type.Name, type);
        ...
        async ValueTask SignalAsync(int frequency = 100)
        {
            ...
            AnsiConsole.Write(new FigletText($"{nameof(Serve)}").LeftJustified().Color(Color.White));
            await new[]
            {
