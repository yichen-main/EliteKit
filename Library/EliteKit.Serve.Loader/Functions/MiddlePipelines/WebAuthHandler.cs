...
using EliteKit.Serve.Loader.Contracts.SystemOverviews;

namespace EliteKit.Serve.Loader.Functions.MiddlePipelines;
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        try
        {
            if (Request.Headers.TryGetValue(HeaderName.Authorization, out var value))
            {
                ...
                {
                    ClaimsIdentity claimsIdentity = new(token.Claims, nameof(EliteKit));
