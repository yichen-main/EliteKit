using EliteKit.Serve.Contracts.WebApplications;
using EliteKit.Serve.Documents.AppSettings;
using EliteKit.Serve.Loader.Functions.MiddlePipelines;
using EliteKit.Serve.Loader.Interfaces.StructuralFrames;

namespace EliteKit.Serve.Loader.Functions.StructuralFrames;
internal sealed class MiddleEdition : IEditionConstructor
{
    public async ValueTask InitialAsync(Assembly assembly)
    {
        ...
        Builder = await new FiberRepository(builder, new TubeRepository(builder)).AddAsync(x =>
        {
            ...
            {
                x.AddFixedWindowLimiter(nameof(EliteKit), x =>
                {
                    x.QueueLimit = 2;
                    x.PermitLimit = 4;
            ...
            static void AddValidator(WebApplicationBuilder builder)
            {
                ...
                builder.Services.AddAuthentication().AddScheme<WebAuthOption, WebAuthHandler>(nameof(EliteKit), option =>
                {
